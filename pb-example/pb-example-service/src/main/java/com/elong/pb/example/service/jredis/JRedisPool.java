...
public class JRedisPool {
    ...
    public JRedisPool(final GenericObjectPoolConfig poolConfig, final String host,
        ...
        if (end != -1) {
            ...
            if (pos != -1) {
                ...
                PlatformLogger.error("JRedis host must have port");
                System.exit(1);
            }
            ...
            PlatformLogger.error("JRedis host must be like A.B.C.D:XXX;A.B.C.D:XXX");
            System.exit(1);
        }
        if (randomRead) {
        ...
        public Object invokeRead(Method method, Object[] args) {
            ...
            while (routeManager.getReadableServers().size() > 0) {
                ...
                try {
                    ...
                    PlatformLogger.error("read exception: " + server.getInfo(), t);

                    if (connectionError) {
                        routeManager.onError(server);
                    } else {
                        PlatformLogger.error("read failed for unknow exception ", t);
    ...
    public void dumpRandomCount() {
        for (int i = 0; i < routeManager.getRandomCount().length(); i++) {
            PlatformLogger.error("random times is " + routeManager.getRandomCount().get(i));
        }
    }
}
